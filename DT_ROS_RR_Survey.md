...
First modify the client from Task 1 to add "Stop" sign detection: taken the fact that the "Stop" sign is almost red, the provided function `detection.py` (takes in BGR format image) filters out all red pixels ([cv2.inRange](https://opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_colorspaces/py_colorspaces.html)) and go through a connected component lableling ([cv2.connectedComponentsWithStats](https://docs.opencv.org/3.4/d3/dc0/group__imgproc__shape.html#ga107a78bf7cd25dec05fb4dfc5c9e765f)). Then integrate this with the client from Task 3: if the centroid of the "stop" sign is below a row threshold, the Duckiebot shall stop. 
...
First modify the subscriber from Task 3 to add "Stop" sign detection: taken the fact that the "Stop" sign is almost red, the provided function `detection.py` (takes in BGR format image) filters out all red pixels ([cv2.inRange](https://opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_colorspaces/py_colorspaces.html)) and go through a connected component lableling ([cv2.connectedComponentsWithStats](https://docs.opencv.org/3.4/d3/dc0/group__imgproc__shape.html#ga107a78bf7cd25dec05fb4dfc5c9e765f)). Then integrate this with the publisher from Task 2: if the centroid of the "stop" sign is below a row threshold, the Duckiebot shall stop.  
